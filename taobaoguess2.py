...
def getgoods(name):
    ...
    params = {"image": img}
    access_token = '[访问百度智能云获得你的acces_token]'
    request_url = request_url + "?access_token=" + access_token
    headers = {'content-type': 'application/x-www-form-urlencoded'}
    response = requests.post(request_url, data=params, headers=headers)
    response = response.json()
    for i in range(0, int(response['words_result_num'])):
        if '晋级概率越大！' in response['words_result'][i]['words']:
            start = i
    for i in range(start + 1, int(response['words_result_num'])):
            myset.add(response['words_result'][i]['words'])
